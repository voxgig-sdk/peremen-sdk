...
[MCP](https://modelcontextprotocol.io) server exposing the Peremen SDK as
two agent tools — `peremen_list` and `peremen_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/peremen-mcp)
make build

# 2. Provide credentials via the environment
export PEREMEN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user peremen \
  -- /absolute/path/to/peremen-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./peremen-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// peremen_list: first page of records
{ "entity": "authentication" }
{ "entity": "authentication", "query": { } }

// peremen_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/peremen-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user peremen \
     -- "$PWD"/dist/*/peremen-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `peremen_list` and `peremen_load` tools now appear
   in new sessions. Ask the agent to *"list authentication using peremen"*
   and it calls `peremen_list` with `{"entity":"authentication"}`.
...
```sh
export PEREMEN_APIKEY=sk_live_xxx            # API key
export PEREMEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `peremen_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `peremen_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PEREMEN_APIKEY` | API key sent with every request. |
| `PEREMEN_BASE` | Optional override of the API base URL. |
...
```sh
./peremen-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"peremen_load","arguments":{"entity":"authentication","query":{"id":1}}}}'
```
