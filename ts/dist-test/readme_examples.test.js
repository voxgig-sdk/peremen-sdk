...
const TEST_SEED = { "entity": { "authentication": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+PeremenSDK\b/.test(code) ||
        /\bPeremenSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PeremenSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PeremenSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
