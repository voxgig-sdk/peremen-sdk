...
module PeremenUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = PeremenResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PeremenResponse.new(fetched)
    else
