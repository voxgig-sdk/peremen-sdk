...
class PeremenMakeError
{
    public static function call(?PeremenContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PeremenContext([], null);
        }
        $op = $ctx->op ?? new PeremenOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PeremenResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PeremenError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PeremenError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof PeremenError) {
