...
boru-driven command-line client **and** interactive REPL for the Peremen
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/peremen-cli)
make build
...
PEREMEN_BASE=https://api.example.com ./peremen-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/peremen-cli
   ```
...
Run with no arguments to open a REPL (prompt `peremen>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/peremen-cli`. |
...
(`./peremen-cli <expr>`) and interactively (the REPL), and expressions compose the
