// Command Peremen-cli is a CLI + REPL for the Peremen SDK,
...
const prompt = "peremen"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "authentication"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PEREMEN_APIKEY (api key), PEREMEN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PEREMEN_APIKEY carries the API key and
	// PEREMEN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PEREMEN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PEREMEN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPeremenSDK(opts)
