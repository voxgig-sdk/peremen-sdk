...
class FtCtrl extends PeremenControl
{
...
class FtClient
{
    ...
    private PeremenUtility $utility;

    public function __construct(PeremenUtility $utility, array $options)
    {
    ...
    public function get_utility(): PeremenUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PeremenContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PeremenContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PeremenConfig::make_feature($name);
    ...
    private static function buildUrl(PeremenSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PeremenError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PeremenResponse($fetched);
        }
    ...
    private function fail(PeremenContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PeremenContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
