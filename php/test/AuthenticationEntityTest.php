...
class AuthenticationEntityTest extends TestCase
{
    ...
    {
        $testsdk = PeremenSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PEREMEN_TEST_AUTHENTICATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function authentication_basic_setup($extra)
{
    ...
    $client = PeremenSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("PEREMEN_TEST_AUTHENTICATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["PEREMEN_TEST_AUTHENTICATION_ENTID"]);
    ...
    if ($env["PEREMEN_TEST_LIVE"] === "TRUE") {
        ...
        $client = new PeremenSDK(Helpers::to_map($merged_opts) ?? []);
    }
