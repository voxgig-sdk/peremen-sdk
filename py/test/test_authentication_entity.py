    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PEREMEN_TEST_AUTHENTICATION_ENTID JSON to run live")
        client = setup["client"]
...
def _authentication_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "PEREMEN_TEST_AUTHENTICATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PEREMEN_TEST_AUTHENTICATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("PEREMEN_TEST_LIVE") == "TRUE":
        ...
        client = PeremenSDK(helpers.to_map(merged_opts))

    _live = env.get("PEREMEN_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("PEREMEN_TEST_EXPLAIN") == "TRUE",
        "live": _live,
