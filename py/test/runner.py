...
class PeremenTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PeremenTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PeremenTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PeremenTestRunner.getenv("PEREMEN_TEST_LIVE")
        override = PeremenTestRunner.getenv("PEREMEN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PeremenTestRunner.getenv(key)
        ...
        explain = PeremenTestRunner.getenv("PEREMEN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PEREMEN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PeremenTestRunner._test_control is not None:
            return PeremenTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PeremenTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PeremenTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PeremenTestRunner.load_test_control()
...
def load_env_local():
    PeremenTestRunner.load_env_local()


def env_override(m):
    return PeremenTestRunner.env_override(m)


def entity_data(v):
    return PeremenTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PeremenTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PeremenTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PeremenTestRunner.load_test_control()


def live_client_options():
    return PeremenTestRunner.live_client_options()


def live_delay_ms():
    return PeremenTestRunner.live_delay_ms()
