...
PeremenSDK._make_feature = _make_feature


function PeremenSDK.new(options)
  local self = setmetatable({}, PeremenSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PeremenSDK:options_map()
...
function PeremenSDK:get_utility()
  return Utility.copy(self._utility)
end


function PeremenSDK:get_root_ctx()
  return self._rootctx
end


function PeremenSDK:prepare(fetchargs)
...
function PeremenSDK:direct(fetchargs)
...
-- by passing it.
function PeremenSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PeremenSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PeremenSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PeremenSDK:Authentication(data)
...
function PeremenSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PeremenSDK.new(sdkopts)
  sdk.mode = "test"
