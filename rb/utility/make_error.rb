...
module PeremenUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PeremenContext.new({}, nil)
    end
    op = ctx.op || PeremenOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PeremenResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PeremenError) ? err.msg : err.to_s
    ...
    sdk_err = PeremenError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PeremenError)
