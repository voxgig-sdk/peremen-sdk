# Peremen SDK Authentication entity
...
class AuthenticationEntity
  ...
  def data_set(args)
    if args
      @_data = PeremenHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PeremenHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Authentication, Hash] the created Authentication; raises PeremenError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = PeremenHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
