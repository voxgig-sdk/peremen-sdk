...
class AuthenticationEntityTest < Minitest::Test
  def test_create_instance
    testsdk = PeremenSDK.test(nil, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set PEREMEN_TEST_AUTHENTICATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def authentication_basic_setup(extra)
  ...
  entid_env_raw = ENV["PEREMEN_TEST_AUTHENTICATION_ENTID"]
  ...
  if env["PEREMEN_TEST_LIVE"] == "TRUE"
    ...
    client = PeremenSDK.new(Helpers.to_map(merged_opts))
  end
