...
func TestAuthenticationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PEREMEN_TEST_AUTHENTICATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func authenticationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PEREMEN_TEST_AUTHENTICATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PEREMEN_TEST_AUTHENTICATION_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["PEREMEN_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewPeremenSDK(core.ToMapAny(mergedOpts))
	}

	live := env["PEREMEN_TEST_LIVE"] == "TRUE"
	return &entityTestSetup{
		client:        client,
		data:          entityData,
		idmap:         idmapResolved,
		env:           env,
		explain:       env["PEREMEN_TEST_EXPLAIN"] == "TRUE",
		live:          live,
