...
import (
	...
	omni "github.com/voxgig-sdk/peremen-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PeremenError); is {
				out := map[string]any{
					"name":    "PeremenError",
...
func omniCtx(arg any, client *sdk.PeremenSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PeremenSDK); is {
			client = live
